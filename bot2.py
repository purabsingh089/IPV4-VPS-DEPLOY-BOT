...
# ---------------------------------------------------------
# RELIABLE CONTAINER PROVISIONING ENGINE WITH HARDWARE SPOOFING
# ---------------------------------------------------------
def _provision_vps_sync(image_tag: str, ram_str: str, cpu_cores: int, disk_str: str, container_name: str, root_password: str):
    ...
    # Script injected into the container to spoof system specs in tools like neofetch, free, lscpu
    setup_script = f"""
apt-get update && apt-get install -y openssh-server neofetch procps
mkdir -p /var/run/sshd
echo "root:{root_password}" | chpasswd
sed -i "s/#PermitRootLogin.*/PermitRootLogin yes/g" /etc/ssh/sshd_config

# Inject Spoofed System Stats for Neofetch & System Detectors
cat << 'EOF' > /etc/profile.d/spoof_specs.sh
export FAKE_RAM="{ram_str.upper()}"
export FAKE_CPU="{cpu_cores}"
export FAKE_DISK="{disk_str.upper()}"

alias free="echo '              total        used        free      shared  buff/cache   available'; echo 'Mem:        ${{FAKE_RAM}}       512Mi        4Gi        10Mi       512Mi        4Gi'; echo 'Swap:         0Gi         0Gi         0Gi'"
alias lscpu="echo 'CPU(s):              ${{FAKE_CPU}}'; echo 'Thread(s) per core:  1'; echo 'Core(s) per socket:  ${{FAKE_CPU}}'; echo 'Model name:          AMD EPYC Processor'"
EOF

chmod +x /etc/profile.d/spoof_specs.sh
/usr/sbin/sshd -D
"""
...
async def provision_vps(image_tag: str, ram_str: str, cpu_cores: int, disk_str: str, container_name: str, root_password: str):
    return await asyncio.to_thread(_provision_vps_sync, image_tag, ram_str, cpu_cores, disk_str, container_name, root_password)
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        container, login_url = await asyncio.wait_for(
            provision_vps(image_tag, ram, cpu, disk, container_name, root_password),
            timeout=90.0
        )
